...
/***/ function(module, exports, __webpack_require__) {
	...
	class Game {
	  ...
	  checkLines() {
	    console.log("check vertical ", this.checkVertical());
	    console.log("check horizontal ", this.checkHorizontal());
	  }
	      ...
	      for (let row = 0; row < 20; row++) {
	        if (this.grid[row][col].color !== -1 &&
	            this.grid[row][col].color === this.grid[row + 1][col].color) {
	              toErase.push(this.grid[row][col]);
	        } else {
	          // toErase = [this.grid[row][col]];
	        }
	      }
	      if (toErase.length >= 4) {
	        return toErase;
